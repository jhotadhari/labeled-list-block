...
class Lali_Labeled_list_block {
	...
	public function on_activate() {
		if ( $this->check_dependencies() ){
			...
			do_action('lali_on_activate_before_flush');
			flush_rewrite_rules();
			$this->maybe_update();
			do_action('lali_plugin_activated');
		} else {
			add_action( 'admin_init', array( $this, 'deactivate' ) );
			wp_die(
	...
	public function on_deactivate() {
		$this->add_roles_and_capabilities();
		do_action('lali_on_deactivate_before_flush');
		flush_rewrite_rules();
		do_action('lali_plugin_deactivated');
	}

	public static function on_uninstall() {
		do_action('lali_plugin_uninstalled');
	}
	...
	protected function init_options() {
		update_option( 'lali_version', self::VERSION );
		add_option( 'lali_db_version', self::DB_VERSION );
	}
	...
	protected function maybe_update() {
		if ( get_option( 'lali_db_version' ) < self::DB_VERSION ) {
			// require_once( self::plugin_dir_path() . 'inc/dep/class-lali-update.php' );
			// new Lali_Update();
			// class Lali_Update()  is missing ??? !!!
		}
	}
...
function lali_labeled_list_block_init() {
	return Lali_Labeled_list_block::get_instance();
}
lali_labeled_list_block_init();
